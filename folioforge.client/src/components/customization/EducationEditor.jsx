...
const EMPTY_ITEM = () => ({ Degree: '', Institution: '', Year: '', GPA: '' });

const FIELDS = [
    { key: 'Degree',      label: 'Degree / Qualification', type: 'text', placeholder: 'e.g. B.Tech in Computer Science' },
    { key: 'Institution', label: 'Institution / University',type: 'text', placeholder: 'e.g. IIT Delhi'                },
    { key: 'Year',        label: 'Year / Period',           type: 'text', placeholder: 'e.g. 2019 – 2023'              },
    { key: 'GPA',         label: 'GPA / Score (optional)',  type: 'text', placeholder: 'e.g. 8.5 / 10'                },
];
        ...
        <div className="flex items-center gap-2 px-3 py-2.5">
            ...
            <button onClick={onToggle} className="flex-1 min-w-0 text-left">
                ...
                <p className="text-xs text-slate-500 truncate">
                    {[item.Institution, item.Year].filter(Boolean).join(' · ') || 'Click to edit'}
                </p>
            </button>
            ...
            <button onClick={() => onRemove(index)} aria-label="Remove education entry" className="p-1 text-slate-600 hover:text-rose-400 transition-colors">
                <Trash2 size={13} />
            </button>
        </div>
 ...
 * EducationEditor
 *
 * Manages an ordered list of education entries.
 * Content shape: `{ items: [{ Degree, Institution, Year, GPA }] }`.
 */
export const EducationEditor = ({ sectionId, content }) => {
