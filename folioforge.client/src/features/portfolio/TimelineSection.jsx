                ...
                <div className="space-y-8">
                    {items.map((item, idx) => (
                        <div key={idx} className="relative pl-12">
                            {/* Timeline dot */}
                            <div className="absolute left-2.5 top-1.5 w-3 h-3 bg-purple-600 rounded-full border-2 border-white dark:border-slate-800 shadow"></div>
                            
                            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-5 border border-slate-100 dark:border-slate-700 hover:shadow-md transition-shadow">
                                <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{item.Role}</h3>
                                <p className="text-purple-600 dark:text-purple-400 font-medium text-sm mb-2">{item.Company}</p>
